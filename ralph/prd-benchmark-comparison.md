...
Текущие бенчмарки сравнивают HTMLParser (Gumbo) только с NSAttributedString(html:). Этого мало — непонятно, как мы выглядим на фоне альтернативных HTML-парсеров (SwiftSoup, swift-justhtml, Lexbor) и подходов (BonMot). Замеряем только парсинг, а пользователь видит pipeline целиком: парсинг + рендеринг.
...
**Описание:** Как разработчик, хочу видеть скорость парсинга Lexbor для сравнения с Gumbo и Swift-парсерами.
